...
private class FirCallArgumentsProcessor(
    ...
    private fun findParameterByName(argument: FirNamedArgumentExpression): FirValueParameter? {
        ...
        fun List<FirValueParameter>.findAndReportValueParameterWithDifferentName(): ProcessorAction {
            val someParameter = getOrNull(matchedIndex)
        ...
        if (parameter == null) {
            if (symbol != null && function.isSubstitutionOrIntersectionOverride) {
                var allowedParameters: List<FirValueParameter>? = null
                (originScope as? FirTypeScope)?.processOverriddenFunctions(symbol) {
                    ...
                    val someParameters = it.fir.valueParameters
                    if (matchedIndex != -1) {
                        someParameters.findAndReportValueParameterWithDifferentName()
                    } else {
                        matchedIndex = someParameters.indexOfFirst { originalParameter ->
                            originalParameter.name == argument.name
                        }
                        if (matchedIndex != -1) {
                            parameter = parameters[matchedIndex]
                            val someParameter = allowedParameters?.getOrNull(matchedIndex)
                            ...
                            allowedParameters = someParameters
                            ProcessorAction.NEXT
                        }
                    }
                }
            ...
            if (symbol != null && function.isIntersectionOverride) {
                ...
                if (matchedIndex != -1) {
                    (originScope as? FirTypeScope)?.processOverriddenFunctions(symbol) {
                        ...
                        it.fir.valueParameters.findAndReportValueParameterWithDifferentName()
                    }
                }
            }
        }
